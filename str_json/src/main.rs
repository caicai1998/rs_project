...
impl<'de> Deserialize<'de> for Sex {
        ...
        D: Deserializer<'de>,
    {
        ...
        match s {
            Some(sex_str) => match sex_str.as_str() {
                "man" => Ok(Sex::Man),
                "woman" => Ok(Sex::Woman),
                _ => Ok(Sex::Unknown),
            },
...
fn main() -> SerdeResult<()> {
    ...
    let input2 = r#"{"person_name": "cjx", "Age": 25, "email": "123@qq.com","sex":"man"}"#;
...
mod tests {
    ...
    #[test]
    fn test_deserialize_person_with_sex() {
        let input = r#"{"person_name": "cjx", "Age": 25, "email": "123@qq.com", "sex": "man"}"#;
        let person: Person = serde_json::from_str(input).unwrap();
        assert_eq!(person.age, 25);
        assert_eq!(person.email, "123@qq.com");
        assert_eq!(person.sex, Some(Sex::Man));
    }
    #[test]
    fn test_deserialize_person_without_sex() {
        let input = r#"{"person_name": "cjx", "Age": 25, "email": "123@qq.com"}"#;
        let person: Person = serde_json::from_str(input).unwrap();
        assert_eq!(person.age, 25);
        assert_eq!(person.email, "123@qq.com");
    ...
    fn test1() {
        ...
        let output2 = serde_json::to_string(&a);
        // println!("{:#?}", a);
        println!("{:#?}", output2);
    }
}
